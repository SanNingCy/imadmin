...
function openToCurrentTab(obj) {
    ...
    if (isScrollToTop) {
        $(obj).show().siblings('.Web4x_iframe').hide();
    } else {
        $(obj).css({"visibility": "visible", "position": "static"}).siblings('.Web4x_iframe').css({"visibility": "hidden", "position": "absolute", "left": "0", "top": "0"});
    }
}
...
$(function() {
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function() {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .Web4x_iframe').each(function() {
                        if ($(this).data('id') == dataUrl) {
                            openToCurrentTab(this);
                            return false;
                        }
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="Web4x_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-refresh="' + isRefresh + '" seamless></iframe>';
            if (isScrollToTop) {
                $('.mainContent').find('iframe.Web4x_iframe').hide();
            } else {
                $('.mainContent').find('iframe.Web4x_iframe').css({"visibility": "hidden", "position": "absolute", "left": "0", "top": "0"});
            }
    ...
    function menuBlank() {
    	// 新窗口打开外网以http://开头，如http://web4x.vip
    	var dataUrl = $(this).attr('href');
    	window.open(dataUrl);
    	return false;
    }
    ...
    function refreshTab() {
    	...
    	var target = $('.Web4x_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.Web4x_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            if (isScrollToTop) {
                $('.Web4x_iframe[data-id="' + $(this).data('id') + '"]').show();
            } else {
                $('.Web4x_iframe[data-id="' + $(this).data('id') + '"]').css({"visibility": "visible", "position": "static"});
            }
    ...
    $.contextMenu({
        ...
        items: {
            ...
            "full": {
                ...
                callback: function(key, opt) {
                    setActiveTab(this);
                    var target = $('.Web4x_iframe[data-id="' + this.data('id') + '"]');
                    target.fullScreen(true);
                }
            },
            "refresh": {
