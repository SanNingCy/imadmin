...
function imCreditCommonConfigFormatVipSwitch(val, row) {
    var v = val;
    if (v == null && row) {
        v = row.vipRedeemEnabled != null ? row.vipRedeemEnabled : row.vip_redeem_enabled;
    }
    return Number(v) === 1 ? "开启" : "关闭";
}

function imCreditCommonConfigFormatVipPurchaseSwitch(val, row) {
    var v = val;
    if (v == null && row) {
        v = row.vipPurchaseEnabled != null ? row.vipPurchaseEnabled : row.vip_purchase_enabled;
    }
    return Number(v) === 1 ? "开启" : "关闭";
}
...
function imCreditCommonConfigFillModal(info) {
    ...
    $("#credit-common-config-vipRedeemEnabled").val(info.vipRedeemEnabled != null ? String(info.vipRedeemEnabled) : (info.vip_redeem_enabled != null ? String(info.vip_redeem_enabled) : "1"));
    $("#credit-common-config-vipPurchaseEnabled").val(info.vipPurchaseEnabled != null ? String(info.vipPurchaseEnabled) : (info.vip_purchase_enabled != null ? String(info.vip_purchase_enabled) : "1"));
...
function imCreditCommonConfigValidate() {
    ...
    if ($("#credit-common-config-vipRedeemEnabled").val() === "") {
        $.modal.alertWarning("请选择会员兑换码开关");
        return false;
    }
    if ($("#credit-common-config-vipPurchaseEnabled").val() === "") {
        $.modal.alertWarning("请选择会员链上购买开关");
        return false;
    }
    return true;
}
...
function imCreditCommonConfigSave(layerIndex) {
    ...
    var vipRedeemEnabled = imCreditCommonConfigParseIntVal("#credit-common-config-vipRedeemEnabled");
    if (vipRedeemEnabled == null) vipRedeemEnabled = 1;
    var vipPurchaseEnabled = imCreditCommonConfigParseIntVal("#credit-common-config-vipPurchaseEnabled");
    if (vipPurchaseEnabled == null) vipPurchaseEnabled = 1;
    ...
    var payload = {
        ...
        vipRedeemEnabled: vipRedeemEnabled,
        vipPurchaseEnabled: vipPurchaseEnabled
    };
...
function imCreditCommonConfigInitTable(canView, canEdit, canDelete) {
    ...
    imInitTable({
        ...
        columns: [
            ...
            { field: "vipRedeemEnabled", title: "会员兑换码开关", sortable: true, formatter: imCreditCommonConfigFormatVipSwitch },
            { field: "vipPurchaseEnabled", title: "会员链上购买开关", sortable: true, formatter: imCreditCommonConfigFormatVipPurchaseSwitch },
