...
var refreshItem = function(){
    ...
    var target = $('.Web4x_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(dataId){
	...
	if ($.common.isNotEmpty(dataId)) {
	    ...
	    // 移除相应tab对应的内容区
	    $('.mainContent .Web4x_iframe[data-id="' + dataId + '"]', topWindow).remove();
	    return;
	}
	...
	if ($.common.isNotEmpty(panelUrl)) {
	    ...
	    $('.mainContent .Web4x_iframe', topWindow).each(function() {
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="Web4x_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        if (isScrollToTop) {
            $('.mainContent', topWindow).find('iframe.Web4x_iframe').hide();
        } else {
            $('.mainContent', topWindow).find('iframe.Web4x_iframe').css({"visibility": "hidden", "position": "absolute", "left": "0", "top": "0"});
        }
...
function activeWindow() {
    ...
    return $('.Web4x_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
}
