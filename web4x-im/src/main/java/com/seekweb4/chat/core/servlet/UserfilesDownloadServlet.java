package com.seekweb4.chat.core.servlet;
...
public class UserfilesDownloadServlet extends HttpServlet {
	...
	public void fileOutputStream(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int index = filepath.indexOf(AppProperites.USERFILES_BASE_URL);
		if(index >= 0) {
			filepath = filepath.substring(index + AppProperites.USERFILES_BASE_URL.length());
		}
		filepath = UriUtils.decode(filepath, "UTF-8");
		AppProperites appProperites = SpringContextHolder.getBean(AppProperites.class);
		File file = new File(appProperites.getUserfilesBaseDir() + AppProperites.USERFILES_BASE_URL + filepath);
