 ...
 * 用户加入频道事件处理策略
 *
 * <p>处理用户加入RTC频道的webhook事件，包括：</p>
 * <ul>
 *   <li>通过频道管理服务更新频道信息</li>
 *   <li>通过用户管理服务记录用户会话</li>
...
public class UserJoinChannelStrategy implements WebhookEventProcessStrategy {
    @Resource
    @Lazy()
    ...
    public Boolean process(EventCallBackReq event) {
        log.info("开始处理观众加入频道事件: noticeId={}, appId={}", event.getNoticeId(), event.getProductId());
        ...
        try {
            ...
            log.error("观众加入频道事件处理失败: {}", e.getMessage(), e);
        }
        return Boolean.TRUE;
    }
    ...
    public ChannelEventTypeEnum getSupportedEventType() {
        return ChannelEventTypeEnum.USER_JOIN_CHANNEL;
    }
