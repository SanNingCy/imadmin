...
@Slf4j
@RestController
@RequestMapping(value = "/roomAdmin/meeting/stats", produces = MediaType.APPLICATION_JSON_VALUE)
public class MeetingStatsAdminController {

    @Resource(name = "meetingStatsServiceImpl")
     ...
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/page</p>
     * 
     * @param req 查询请求
     * @return 分页响应
     */
    @PostMapping("/page")
    @ResponseBody
    public AjaxJson page(@RequestBody MeetingQueryReq req) {
        try {
     ...
     * 
     * <p><strong>HTTP方法：</strong>POST</p>
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/durationRoomMeeting</p>
     * 
     * @param req 查询请求
     * @return 会议室详情
     */
     ...
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/duration</p>
     * 
     ...
     * 
     * <p><strong>HTTP方法：</strong>GET</p>
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/active/count</p>
     * 
     * @return 正在进行中的会议数量
     */
    ...
    public AjaxJson getActiveMeetingCount() {
        try {
            ...
            long activeCount = meetingRepository.countByStatusIn(activeStatuses);
            ...
            result.put("description", "正在进行中的会议数量（包括active和inactive状态）");
            
            log.info("查询正在进行中的会议数量：{}", activeCount);
            return AjaxJson.success().put("data", result);
        } catch (Exception e) {
     ...
     * 
     * <p><strong>HTTP方法：</strong>GET</p>
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/active/count/{groupId}</p>
     * 
     * @param groupId 群组ID
    ...
    public AjaxJson getActiveMeetingCountByGroup(@PathVariable String groupId) {
        try {
            ...
            // 正在进行中的会议状态：active 和 inactive
            List<String> activeStatuses = Arrays.asList("active", "inactive");
            long activeCount = meetingRepository.countByGroupIdAndStatusIn(groupId, activeStatuses);
            ...
            result.put("queryTime", System.currentTimeMillis());
            result.put("statuses", activeStatuses);
            result.put("description", "指定群组正在进行中的会议数量（包括active和inactive状态）");
            
            log.info("查询群组 {} 正在进行中的会议数量：{}", groupId, activeCount);
            return AjaxJson.success().put("data", result);
        } catch (Exception e) {
     ...
     * 
     * <p><strong>HTTP方法：</strong>POST</p>
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/statistics</p>
     * 
     * @param req 查询请求
     * @return 统计信息
     */
     ...
     * 
     * <p><strong>HTTP方法：</strong>GET</p>
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/user/{ownerId}</p>
     * 
     * @param ownerId 用户ID
     * @param startTime 开始时间（可选，格式：yyyy-MM-dd）
     * @param endTime 结束时间（可选，格式：yyyy-MM-dd）
     ...
     * 
     * <p><strong>HTTP方法：</strong>GET</p>
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/status/{status}</p>
     * 
     * @param status 会议状态
     * @param startTime 开始时间（可选，格式：yyyy-MM-dd）
     * @param endTime 结束时间（可选，格式：yyyy-MM-dd）
     ...
     * 
     * <p><strong>HTTP方法：</strong>GET</p>
     * <p><strong>请求路径：</strong>/roomAdmin/meeting/stats/duration/ongoing</p>
     * 
     * @param startTime 开始时间（可选，格式：yyyy-MM-dd）
     * @param endTime 结束时间（可选，格式：yyyy-MM-dd）
     * @return 进行中会议时长统计
    ...
    public AjaxJson getOngoingMeetingDurationStats(@RequestParam(required = false) String startTime,
                                                   @RequestParam(required = false) String endTime) {
        try {
            log.info("获取进行中会议时长统计，时间范围：{} - {}", startTime, endTime);
            ...
            result.put("queryTime", System.currentTimeMillis());
            result.put("description", "进行中会议时长统计（包括active和inactive状态）");
