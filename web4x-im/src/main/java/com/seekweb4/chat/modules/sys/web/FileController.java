...
import com.seekweb4.chat.modules.sys.entity.FileData;
import com.seekweb4.chat.modules.sys.utils.FileKit;
...
//			response.setHeader("Content-Disposition", "attachment; filename=" + file.getName());
	...
	public AjaxJson getPolicy() {
		return AjaxJson.success(aliyunOSSRepository.getPolicy());
	}*/
	/**
	 * 下载文件
	 * @return
	...
	public AjaxJson webupload( HttpServletRequest request, HttpServletResponse response,MultipartFile file) {
        AjaxJson j = new AjaxJson();
		// 判断文件是否为空
		if (!file.isEmpty()) {
			...
			if(fileProperties.isAvailable(name)) {
				...
				String url = accessoryRepository.save(file, UserUtils.getUser().getId() + "/" + uploadPath, name);
				j.put ("id", name);
				j.put ("url", url);
				return j;
			} else {
	 ...
	 */
	@RequiresPermissions("user")
	@RequestMapping("editorUploadImage")
	public Map<String, Object> editorUploadImage(MultipartFile[] files) {
		...
		for (MultipartFile file : files) {
			if (!file.isEmpty()) {
				// String name = file.getOriginalFilename ();
				String name = System.currentTimeMillis() + "." + StringUtils.substringAfterLast(file.getOriginalFilename(), ".");
				if(fileProperties.isImage(name)) {
					String url = accessoryRepository.save(file, "editorImage", name);
					data.add(getRealPath(url));
				} else {
					map.put("errno", 1);
					map.put("msg", "请勿上传非法文件!");
					return map;
				}
			}
		}
	 ...
	 */
	@RequiresPermissions("user")
	@RequestMapping("editorUploadVideo")
	public Map<String, Object> editorUploadVideo(MultipartFile file) {
		Map<String, Object> map = Maps.newHashMap();
		map.put("errno", 0);
		...
		if (!file.isEmpty()) {
			// String name = file.getOriginalFilename ();
			String name = System.currentTimeMillis() + "." + StringUtils.substringAfterLast(file.getOriginalFilename(), ".");
			if(fileProperties.isVideo(name)) {
				String url = accessoryRepository.save(file, "editorVideo", name);
				data.put("url", getRealPath(url));
			} else {
				map.put("errno", 1);
				map.put("msg", "请勿上传非法文件!");
				return map;
