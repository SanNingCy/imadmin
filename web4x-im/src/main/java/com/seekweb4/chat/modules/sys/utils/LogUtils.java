...
import com.seekweb4.chat.common.utils.Exceptions;
...
import com.seekweb4.chat.modules.sys.mapper.LogMapper;
		...
		public void run() {
			...
			if (StringUtils.isBlank(log.getTitle())){
				...
				if (handler instanceof HandlerMethod){
					...
					// 注解日志
					ApiLog aopLog = m.getAnnotation(ApiLog.class);
					if (aopLog != null) {
		...
		if (menuMap == null){
			...
			for (Menu menu : menuList){
				...
				if (StringUtils.isNotBlank(menu.getPath())){
					menuMap.put(menu.getPath(), namePath);
